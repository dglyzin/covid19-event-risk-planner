...
event_size = c(50,100,500,1000,5000) #c(10, 15, 20, 25, 50, 100, 500, 1000, 5000)
asc_bias_list <<- c(10) #c(3, 5, 10)
...
for (asc_bias in asc_bias_list) {
  ...
  for (size in event_size){
    ...
    map <- leaflet() %>%
      ...
      addLegend(
        data = russia_riskdt_map,
        position = "topright", pal = russia_pal, values = ~risk,
        title = "Risk Level (%)",
        opacity = 0.7,
        labFormat = function(type, cuts, p) {
          paste0(russia_legendlabs)
        }
      )
    
    # addLegend( ---------------------- это старая легенда для UK
    #   data = uk_riskdt_map,
