...
#post_tweet(status = paste0("Sweden province-level risk estimate update for ",  now("Europe/Zurich"), "UTC .  Estimated risk that at least 1 person is #COVID19 positive for events or other areas where ", size, " individuals are in close contact [Assuming 5:1 ascertainment bias]"),
# media = file.path(getwd(), "daily_risk_map_sweden", current_time, paste0(current_time,"_", asc_bias, "_", size, ".png")))
